#[cfg_attr(all(any(target_arch = "x86", target_arch = "x86_64"), target_feature = "sse2"), path = "x86.rs")]
#[cfg_attr(all(target_arch = "aarch64", target_feature="fp"), path = "aarch64.rs")]
mod inner;
pub use self::inner::*;
...
mod tests {
    ...
    #[test]
    fn test_aes() {
        let value = m128i::from_i32(100000000, 200000000, 3, 4);
        let key = m128i::from_i32(100000000, 200000000, 3, 4);
        let value_d = value.aesenc(key);
        println!("{:?}", value_d);
    }
    ...
    fn test_m128i_to_m128d() {
        let value = m128i::from_i32(100000000, 200000000, 3, 4);
        let value_d = value.to_m128d();
        let result = m128d::from_f64(3.0,4.0);
        assert_eq!(value_d, result);
        println!("{:?}", value_d);
    }
    #[test]
    ...
    fn test_m128d_shuffle_1() {
        ...
        let expected = m128d::from_f64(65638.3748, 788.888);
        println!("Expected   {:x}", expected);
        assert_eq!(m_shuffled, expected); 
    }
    ...
    fn test_m128d_xor() {
        ...
        let expected = m128d::from_u64(0x0FFFFFFFFFFFFFFF, 0x0FFFFFFFFFFFFFFF);
        assert_eq!(m_xored, expected);

        let nulled = m1 ^ m1;
        let expected = m128d::from_u64(0x0, 0x0);
        assert_eq!(nulled, expected);    
    }
